# Aura — App Store Connect Privacy formulier
...
Dit document mapt elke Apple-vraag naar het juiste antwoord voor Aura.
...
Hoewel Aura's kerfunctionaliteit volledig lokaal is (geen data verlaat
...
| Categorie | Reden |
|---|---|
| Financial Info | Aura verwerkt geen betaling |
...
> 💡 Aura's *lokale* opslag (localStorage) telt **niet** als "collected"
...
```
Aura collects health data (current menstrual cycle phase and cycle
...
### Voor "Email Address"
```
Aura collects email addresses only when the user opts into the
...
third-party SDK. Voor Aura's iOS-app moet je een
...
| Vragen om health-data zonder duidelijke reden | Aura geeft per veld uitleg (zie consent-gate + in-app legal-tekst) |
| Geen opt-out / data-deletion flow | Aura heeft één-knop "wis alles" + JSON-export voor portabiliteit |
