...
function SectionInfoModal({ title, body, onClose }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-ink-900/40 backdrop-blur-sm p-4 pb-8"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className="w-full max-w-md bg-cream-50 rounded-2xl shadow-glow p-6 anim-fade-up"
...
const CAL_DAY_HEADERS_NL = ['Ma', 'Di', 'Wo', 'Do', 'Vr', 'Za', 'Zo'];
const CAL_DAY_HEADERS_EN = ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'];
...
function CycleCalendarCard({ profile, onUpdateProfile, state }) {
  const { t, locale, formatDate } = useT();
  const today = useMemo(() => new Date(), []);
  const grid  = useMemo(() => buildCalendarGrid(profile, today), [profile, today]);
  const [selected, setSelected] = useState(null);
  const [editing, setEditing]   = useState(false);
  const [draftDate, setDraftDate] = useState(() => profile?.lastPeriodStart ? toISODate(profile.lastPeriodStart) : toISODate(today));

  const headers = locale === 'en' ? CAL_DAY_HEADERS_EN : CAL_DAY_HEADERS_NL;
  ...
  const tagLabel = (cell) => {
    if (!cell.tag) return cell.predicted ? '' : t('calendar.tagNone');
    const baseKey = cell.tag === 'period'    ? 'calendar.tagPeriod'
                  : cell.tag === 'fertile'   ? 'calendar.tagFertile'
                  :                            'calendar.tagOvulation';
    const base = t(baseKey);
    return cell.predicted ? t('calendar.tagPredicted', { tag: base }) : base;
  };
  ...
  const openEdit = () => {
    setDraftDate(profile?.lastPeriodStart ? toISODate(profile.lastPeriodStart) : toISODate(today));
    setEditing(true);
  };
  const saveEdit = () => {
    if (!draftDate) return;
    const next = correctCurrentPeriodStart(profile, draftDate);
    onUpdateProfile(next);
    setEditing(false);
  };
    ...
    <Card className="p-6 mb-5 anim-fade-up" style={{ animationDelay: '110ms' }}>
      <div className="flex items-center justify-between mb-4 gap-2">
        <div className="flex items-center gap-1.5 min-w-0">
          <Calendar className="w-3.5 h-3.5 text-ink-400 shrink-0" aria-hidden="true" />
          <div className="text-[11px] uppercase tracking-[0.18em] text-ink-400 truncate">{t('calendar.title')}</div>
        </div>
        {nextStartISO && (
          <div className="text-[11px] text-terracotta-600 bg-terracotta-100 px-2 py-0.5 rounded-full whitespace-nowrap">
            {t('calendar.next', { date: formatShortDate(nextStartISO, formatDate) })}
          </div>
        )}
      </div>

      {state?.hasData && state.cycleDay != null && (
        <div className="text-[12px] text-ink-500 mb-3">
          {t('calendar.currentDay', { day: state.cycleDay, total: state.cycleLength })}
        </div>
      )}
        ...
        {grid.map((cell) => {
          ...
          return (
            <button
              ...
              aria-label={t('calendar.cellAria', { date: cell.iso, tag: tagLabel(cell) || t('calendar.tagNone') })}
      ...
      {tooltipCell && (
        <div className="mt-3 px-3 py-2.5 rounded-xl bg-cream-100/80 border border-cream-200 text-[12px] text-ink-600 leading-snug anim-fade-up">
          ...
          <div className="text-ink-500">{tagLabel(tooltipCell) || t('calendar.tagNoneCell')}</div>
        </div>
      )}

      <div className="flex flex-wrap items-center gap-3 mt-4">
        <LegendDot color={CAL_COLORS.period}    label={t('calendar.legend.period')} />
        <LegendDot color={CAL_COLORS.fertile}   label={t('calendar.legend.fertile')} />
        <LegendDot color={CAL_COLORS.ovulation} label={t('calendar.legend.ovulation')} />
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-full border-2 border-sage-500" aria-hidden="true" />
          <span className="text-[11px] text-ink-500">{t('calendar.legend.today')}</span>
        </div>
      </div>
      ...
      {/* Start-date correction — fixes "Aura thinks I'm on day 3 but I'm on day 9". */}
      <div className="mt-4 pt-4 border-t border-cream-200/70">
        {!editing ? (
          <button
            type="button"
            onClick={openEdit}
            className="w-full inline-flex items-center justify-center gap-2 min-h-[44px] py-2.5 rounded-xl bg-sage-50 border border-sage-200 text-sage-700 text-sm font-medium hover:bg-sage-100 active:scale-[0.98] transition"
          >
            <RefreshCw aria-hidden="true" className="w-4 h-4" />
            {t('calendar.editStart')}
          </button>
        ) : (
          <div className="space-y-3">
            <div>
              <div className="text-[11px] uppercase tracking-[0.14em] text-ink-400 mb-1">{t('calendar.editStart.title')}</div>
              <p className="text-[12px] text-ink-500 leading-snug">{t('calendar.editStart.hint')}</p>
            </div>
            <input
              type="date"
              value={draftDate}
              ...
              className="w-full min-h-[44px] px-3 py-2 rounded-xl border border-cream-200 bg-cream-50 text-ink-700 text-sm focus:outline-none focus:border-sage-300"
              aria-label={t('calendar.editStart.title')}
            />
            <div className="flex gap-2">
              <button
                type="button"
                onClick={cancelEdit}
                className="flex-1 min-h-[44px] py-2.5 rounded-xl border border-cream-200 text-ink-600 text-sm font-medium hover:bg-cream-100 active:scale-[0.98] transition"
              >
                {t('calendar.editStart.cancel')}
              </button>
              <button
                type="button"
                onClick={saveEdit}
                disabled={!draftDate}
                className="flex-1 min-h-[44px] py-2.5 rounded-xl bg-sage-500 text-cream-50 text-sm font-medium hover:bg-sage-600 active:scale-[0.98] transition disabled:opacity-50"
              >
                {t('calendar.editStart.save')}
              </button>
            </div>
          </div>
        )}
...
function Dashboard({ profile, onUpdateProfile, onOpenSettings, onOpenVoeding }) {
  ...
  return (
    <div className="min-h-dvh px-5 py-8 pb-28 max-w-md mx-auto">
      ...
      {/* Goal progress rings — kcal / protein / water / movement at a glance. */}
      {!hidden.has('goals') && <GoalRings log={log} goals={profile.goals} targets={targets} />}

      {/* Cycle calendar — 6-week grid + manual start-date correction. */}
      {!hidden.has('calendar') && (
        <CycleCalendarCard profile={profile} onUpdateProfile={onUpdateProfile} state={state} />
      )}
